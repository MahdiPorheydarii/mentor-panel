...
export default async function ClassesPage() {
  ...
  const totalCompleted = students.reduce((acc, s) => acc + s.classesCompleted, 0);
  const totalClasses = students.reduce((acc, s) => acc + s.totalClasses, 0);
  const overallRate = totalClasses > 0 ? Math.round((totalCompleted / totalClasses) * 100) : 0;
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400">
            نرخ کلی: {overallRate}٪ · {totalCompleted} از {totalClasses} هفته گذرانده‌شده
          </p>
        </div>
      </div>
      ...
      <div className="rounded-xl bg-white border border-slate-100 shadow-sm p-5">
        <div className="flex items-center justify-between mb-3">
          <p className="text-sm font-medium text-slate-700">پیشرفت کلی دانشجویان</p>
          <p className="text-xl font-bold text-blue-600">{overallRate}٪</p>
        </div>
        <div className="h-2.5 w-full rounded-full bg-slate-100 overflow-hidden">
          <div className="h-full rounded-full bg-blue-500 transition-all" style={{ width: `${overallRate}%` }} />
        </div>
      </div>

      <div className="rounded-xl bg-white shadow-sm overflow-hidden border border-slate-100">
        {sorted.length === 0 ? (
          <p className="px-4 py-12 text-center text-slate-400 text-sm">هیچ دانشجو فعالی یافت نشد.</p>
        ) : (
            ...
            <table className="w-full min-w-[600px] text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  ...
                  <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wide text-slate-500">هفته</th>
