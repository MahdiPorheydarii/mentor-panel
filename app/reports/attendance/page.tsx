...
export default function AttendancePage() {
  const sorted = [...mockAttendance].sort((a, b) => b.sessionDate.localeCompare(a.sessionDate));
  const presentCount = sorted.filter((r) => r.present).length;
  const rate = Math.round((presentCount / sorted.length) * 100);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-semibold text-slate-900">حضور و غیاب</h1>
          <p className="text-xs text-slate-400">{sorted.length} جلسه ثبت‌شده · نرخ حضور: {rate}٪</p>
        </div>
      </div>

      {/* Summary */}
      <div className="grid grid-cols-3 gap-4">
        <div className="rounded-xl bg-white border border-slate-100 shadow-sm p-4 text-center">
          <p className="text-2xl font-bold text-slate-900">{sorted.length}</p>
          <p className="text-xs text-slate-500 mt-0.5">کل جلسات</p>
        </div>
        <div className="rounded-xl bg-white border border-slate-100 shadow-sm p-4 text-center">
          <p className="text-2xl font-bold text-emerald-600">{presentCount}</p>
          <p className="text-xs text-slate-500 mt-0.5">حضور</p>
        </div>
        <div className="rounded-xl bg-white border border-slate-100 shadow-sm p-4 text-center">
          <p className="text-2xl font-bold text-red-500">{sorted.length - presentCount}</p>
          <p className="text-xs text-slate-500 mt-0.5">غیاب</p>
        </div>
      </div>
          ...
          <table className="w-full min-w-[600px] text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50">
                ...
                <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wide text-slate-500">نوع جلسه</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {sorted.map((record) => (
                <tr key={record.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-3.5 text-xs text-slate-600">{record.sessionType}</td>
                  <td className="px-4 py-3.5 text-xs text-slate-600">
                    {record.duration ? `${record.duration} دقیقه` : '—'}
                  </td>
