...
export default function HomePage() {
  ...
  const warnings = mockStudents.filter((s) => s.withdrawalWarning).length;
  const incompleteCards = mockStudents.filter((s) => !s.reportCardDone).length;
  ...
  const stats = [
    ...
    {
      label: 'کارنامه‌های ناقص',
      value: incompleteCards,
      icon: ClipboardX,
      color: 'text-red-600',
      bg: 'bg-red-50',
    },
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="mb-4 flex items-center justify-between">
          ...
          <p className="text-xs text-slate-400">به‌روزرسانی لحظه‌ای</p>
        </div>
        <StudentsTable />
      </div>
    </div>
